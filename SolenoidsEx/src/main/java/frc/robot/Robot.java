...
public class Robot extends TimedRobot {
  ...
  private final Solenoid m_solenoid = new Solenoid(Constants.Solenoid.kModuleType, Constants.Solenoid.kChannel);

  // DoubleSolenoid corresponds to a double solenoid.
  // In this case, it's connected to channels 1 and 2 of a PH with the default CAN ID.
  private final DoubleSolenoid m_doubleSolenoid =
      new DoubleSolenoid(Constants.Solenoid.kModuleType, Constants.Solenoid.kFwdChannel, Constants.Solenoid.kREvChannel);

  // Compressor connected to a PH with a default CAN ID (1)
  private final Compressor m_compressor = new Compressor(Constants.Solenoid.kModuleType);
  ...
  public void teleopPeriodic() {
     ...
     */
    m_solenoid.set(m_stick.getRawButton(Constants.Joystick.kSolenoidButton));
     ...
     */
    if (m_stick.getRawButtonPressed(Constants.Joystick.kDoubleSolenoidForwardButton)) {
      m_doubleSolenoid.set(DoubleSolenoid.Value.kForward);
    } else if (m_stick.getRawButtonPressed(Constants.Joystick.kDoubleSolenoidReverseButton)) {
      m_doubleSolenoid.set(DoubleSolenoid.Value.kReverse);
    }

    // On button press, toggle the compressor.
    if (m_stick.getRawButtonPressed(Constants.Joystick.kCompressorButton)) {
